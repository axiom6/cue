...
export default {
  ...
  methods: {
    calcSize () {
      ...
      this.maxHeight = parseFloat(this.wrapper.offsetHeight)
      this.contentHeight = parseFloat(this.content.offsetHeight)
      this.trackHeight = parseFloat(this.track.offsetHeight)
    ...
    scroll (e) {
      ...
      //if (browser.name === 'firefox') { delta *= 10 }
      this.setVertical(delta)
      e.preventDefault()
    },
    setVertical (delta) {
      ...
      let currentMT = this.content.style.marginTop === ''
        ? 0
        : parseFloat(this.content.style.marginTop)
      let nextMT = 0
